...
The experiment results are shown in Table~\ref{res:ner}. Compare the effectiveness of Bi-LSTM model and Uni-LSTM model, we can observe that Bi-LSTM has better F1 score~(+11.03) than the Uni-LSTM model, which is a large gap in Entity Detection task. To order to quantify what kind of error does Uni-LSTM make, we conduct further error analysis. We select those examples that Bi-LSTM makes correct predictions and but Uni-LSTM fails, as shown in \ref{tab:bi-uni}. We categorize the cases from three dimensions. First, whether the token is an entity or an ordinary word according to golden label, and whether the token is in the vocabulary. The third dimension is from hand annotation. We sampled 50 cases from 1393 cases and annotate to further categorize the error source. We give each an example for each case and the target tokens are in bold form.
...
\begin{itemize}
	...
	\item OOV: some words are not in the Glove vocabulary
\end{itemize}

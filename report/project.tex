...
\begin{table}[t]
	...
	\end{tabular}
	\vspace{3mm}
	\caption{Entity Detection performance on OntoNotes 5 English Dataset Test set. Uni-LSTM denotes the Uni-LSTM. MLP(bi) denotes the MLP model based on the Bi-LSTM embeddings. MLP(uni) denotes the MLP model based on Uni-LSTM embeddings.}
	\label{res:ner}
\end{table}
...
Ordinary-Vocab		& 239		&14		& For \textbf{more} information check out : www.taconet.com.tw/cstudio/\\
Ordinary-OOV		& 117		&10		& Dr. Konan Peck , an assistant research fellow at Academia Sinica 's Institute of Biomedical Sciences -LRB- IBMS -RRB- , says that \textbf{biochips} need to be made more widely available as a molecular biology research tool .\\
Entity-Vocab		& 823		&38		& In \textbf{the} first quarter of this year , the development of Guangzhou 's privately owned enterprises again showed many characteristics : all walks of life are developing extensively , and the development of industry and commerce is remarkable .\\
Entity-OOV		& 214		&13		& \textbf{Guofang} Shen said that this bill is a bill carrying political discrimination and extremely unfriendly to China .\\
\hline
...
\hline\hline
category		&Total \# case 	& Sample \# case &examples \\
OEOTrue		& 418	&14		& \textbf{Electrical} and high science and technology products grew considerably.\\
OEOFalse		&1675	&47		& Another popular technique involves covering the trees with black \textbf{mosquito} nets to improve yields.\\
EOETrue		& 1363	&34		& David any reaction from the Bush team on the Democrats ' plan to contest the vote in Miami \textbf{-} Dade?\\
EOEFalse		& 0		&0		& - \\
\hline
...
To examine the source of effectiveness of LSTM, we conducted an experiment that randomly masks the entities in test data. To eliminate the affect of word embedding and measure the contribution from context, we mask the entities to be ``[$<$UNK$>$]''. The results can be found in \ref{tab:mask}
\begin{table}[t]
	\centering
	\begin{tabular}{l@{\qquad}ccc}
		\toprule
		...
		Uni-LSTM-forward 	& 72.47 			& 73.88 			& 73.17 \\
		Uni-LSTM-backward & 80.31 			& 80.28 			& 80.29 \\ \midrule
		Bi-LSTM-masked 	& 64.53 			& 33.74			& 44.31 \\ 
		Uni-LSTM-forward-masked & 67.01 		& 27.98 			& 39.48 \\ 
...
We can see that after masking the entities to be ``[$<$UNK$>$]'', the recall drop dramatically. This means LSTM fails to detect entities without the embedding information from the word itself. This give us the idea that most effectiveness of sequence tagging may come from the word embedding. So we further analysis the embedding using MLP models.

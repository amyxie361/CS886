...
def eval(model, crit, data, vocab={}, args={}):
    ...
    for (x, x_mask, y) in data:
        ...
        for i in range(B):
            ...
            #print("<p>predicted: " + str(pred_y.data[i].item()) + " correct label: " + str(y.data[i].item()))
...
if __name__ == "__main__":
    ...
    while 1:
        ...
        if args.use_cuda:
            loss = loss.cpu()
        if iteration % 5 == 0:
            print("iteration %d training loss: %f" %(iteration, loss.data.item()))
        iteration += 1
        # code.interact(local=locals())
        if iteration % args.eval_iterations == 0:
            dev_loss, dev_correct_count, dev_count = eval(model, crit, dev_data, vocab=vocab, args=args)
            print(dev_correct_count)
            print(dev_count)
            dev_acc = dev_correct_count.item()/dev_count
            dev_loss = dev_loss/dev_count
            print("dev accuracy: %f, dev loss: %f" % (dev_acc, dev_loss))
            if dev_acc > best_dev_acc:
                best_dev_acc = dev_acc
                ...
                print("model saved.")
            if iteration % 2000 == 0:
                torch.save(model, args.model_file + str(iteration))
                print("model saved.")

...
def test(model, crit, data, vocab={}, index2word={}, args={}):
	...
	for (x, x_mask, y) in data:
		...
		for i in range(B):
			f_extract_lstm.write("<p>predicted: " + str(pred_y.data[i].item()) + " correct label: " + str(y.data[i].item()))
			for j in range(lengths[i]):
				# code.interact(local=locals())
				if negative_indicator[i].item() == j:
					# print("negative")
					f_extract_lstm.write("<span style='font-size:" + str(out[i][j][pred_y.data[i]].item()) + \
						";color:green' title='" + str(out[i][j][pred_y.data[i]].item()) + "'>" + str(index2word[x.data[i][j].item()]) + " </span>")
				elif positive_indicator[i].item() == j:
					# print("positive")
					f_extract_lstm.write("<span style='font-size:" + str(out[i][j][pred_y.data[i]].item()) + \
						";color:red' title='" + str(out[i][j][pred_y.data[i]].item()) + "'>" + str(index2word[x.data[i][j].item()]) + " </span>")
				else:
					f_extract_lstm.write("<span style='font-size:" + str(out[i][j][pred_y.data[i]].item()) + \
						"' title='" + str(out[i][j][pred_y.data[i]]) + "'>" + str(index2word[x.data[i][j].item()]) + " </span>")
			# print("<span style='font-size:" + str(out[i][j][pred_y.data[i]].item()) + \
			# 		"' title='" + str(out[i][j][pred_y.data[i]].item()) + "'>" + str(index2word[x.data[i][j].item()]) + " </span>")
		...
		total_count += B
		loss = crit(pred, y)
		total_loss += loss.data.item() * total_count
...
if __name__ == "__main__":
	...
	test_acc = test_correct_count.item()/test_count
